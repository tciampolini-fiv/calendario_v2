...
function onEdit(e){
  ...
  if(name===EVENT_APP.SHEETS.TASKS){eventHandleTaskEditV9_(e);return;}
...
function eventTaskStatusFormulaV9_(row){return '=IF(A'+row+'="";"";IF(D'+row+'="";"DA FARE";IFERROR(IF(AND(INDEX($E$2:$E$500;MATCH(D'+row+';$C$2:$C$500;0))="FATTO";$F'+row+'<>"";$F'+row+'<=TODAY());"DA FARE";"IN ATTESA");"IN ATTESA")))';}

function eventHandleTaskEditV9_(e){
  const sheet=e.range.getSheet(),firstRow=Math.max(2,e.range.getRow()),lastRow=Math.min(500,e.range.getLastRow()),firstCol=e.range.getColumn(),lastCol=e.range.getLastColumn();
  if(lastRow<firstRow||firstCol>6)return;
  if(firstCol<=1&&lastCol>=1)for(let row=firstRow;row<=lastRow;row++)eventPrepareTaskRowV9_(sheet,row);
  if(firstCol<=4&&lastCol>=4)for(let row=firstRow;row<=lastRow;row++)eventApplyTaskDependencyV9_(sheet,row);
  if(firstCol<=5&&lastCol>=5){
    for(let row=firstRow;row<=lastRow;row++)if(eventNormalizeTextV7_(sheet.getRange(row,5).getDisplayValue())==='FATTO')eventScheduleDependentTasksV9_(sheet,row);
    eventRefreshTaskDependencyStatesV9_();
  }
...
function eventPrepareTaskRowV9_(sheet,row){
  ...
  eventAutoLinkConfirmationTaskRowV9_(sheet,row);eventApplyTaskDependencyV9_(sheet,row);
}

function eventApplyTaskDependencyV9_(sheet,row){
  const description=String(sheet.getRange(row,1).getDisplayValue()||'').trim();if(!description)return;
  const status=eventNormalizeTextV7_(sheet.getRange(row,5).getDisplayValue());if(status==='FATTO')return;
...
function eventRefreshTaskDependencyStatesV9_(){
  ...
  const last=Math.min(500,Math.max(sheet.getLastRow(),2)),values=sheet.getRange(2,1,last-1,6).getValues(),rowByNo={};
  ...
  for(let row=2;row<=last;row++){
    ...
    const status=eventNormalizeTextV7_(sheet.getRange(row,5).getDisplayValue());if(status==='FATTO')continue;
    const depNo=Number(sheet.getRange(row,4).getValue()||0);
    if(depNo>0){
      const parentRow=rowByNo[depNo]||0,parentDone=parentRow>0&&eventNormalizeTextV7_(sheet.getRange(parentRow,5).getDisplayValue())==='FATTO',due=sheet.getRange(row,6).getValue();
      if(parentDone&&due instanceof Date&&eventDateReachedV9_(due))sheet.getRange(row,5).setValue('DA FARE');
      else sheet.getRange(row,5).setFormula(eventTaskStatusFormulaV9_(row));
    }else if(!status)sheet.getRange(row,5).setValue('DA FARE');
  }
}

function eventDateReachedV9_(date){if(!(date instanceof Date))return false;const due=new Date(date),today=new Date();due.setHours(0,0,0,0);today.setHours(0,0,0,0);return due.getTime()<=today.getTime();}

function eventScheduleDependentTasksV9_(sheet,parentRow){
  const parentNo=Number(sheet.getRange(parentRow,3).getValue()||0);if(!(parentNo>0))return;
  const last=Math.min(500,Math.max(sheet.getLastRow(),2)),rows=sheet.getRange(2,1,last-1,6).getValues();
  rows.forEach((r,i)=>{
    if(Number(r[3]||0)!==parentNo)return;
    const row=i+2,description=eventNormalizeTextV7_(r[0]),dueCell=sheet.getRange(row,6),existingDue=dueCell.getValue();
    if(description!=='CONFERMA PRESENZE TUTTI ATLETI'&&!(existingDue instanceof Date)){const due=new Date();due.setHours(12,0,0,0);due.setDate(due.getDate()+2);dueCell.setValue(due).setNumberFormat('dd/MM/yyyy');}
    if(eventNormalizeTextV7_(sheet.getRange(row,5).getDisplayValue())!=='FATTO')sheet.getRange(row,5).setFormula(eventTaskStatusFormulaV9_(row));
  });
}

function eventAutoLinkConfirmationTasksV9_(){const sheet=SpreadsheetApp.getActive().getSheetByName(EVENT_APP.SHEETS.TASKS);if(!sheet)return;for(let row=2;row<=Math.min(500,Math.max(sheet.getLastRow(),2));row++)eventAutoLinkConfirmationTaskRowV9_(sheet,row);}
function eventAutoLinkConfirmationTaskRowV9_(sheet,row){
  if(Number(sheet.getRange(row,4).getValue()||0)>0)return;
  const desc=eventNormalizeTextV7_(sheet.getRange(row,1).getDisplayValue());if(!desc)return;
  const parents={'CONFERMA GOMMONE':'RICHIESTA GOMMONE','CONFERMA SOGGIORNO':'RICHIESTA SOGGIORNO','CONFERMA PASTI':'SOLUZIONE PASTI','CONFERMA PRESENZE TUTTI ATLETI':'CONVOCAZIONE ATLETI','CONFERMA OSPITALITA CIRCOLO':'OSPITALITA CIRCOLO'},parent=parents[desc]||'';if(!parent)return;
  const values=sheet.getRange(2,1,Math.max(1,row-2),3).getDisplayValues();
  for(let i=values.length-1;i>=0;i--)if(eventNormalizeTextV7_(values[i][0])===parent&&Number(values[i][2]||0)>0){sheet.getRange(row,4).setValue(Number(values[i][2]));return;}
}

...
function prepareEventSheetForSelectedEventV2() {
  ...
  SpreadsheetApp.getUi().alert(
    ...
    result.created?'È stata creata una nuova Scheda evento con task, IMP e tecnici già inizializzati.':'La Scheda evento collegata è stata aggiornata senza modificarne la struttura.',
...
function isEventSheetV8_(child) {
  const expenses=child&&child.getSheetByName(EVENT_SHEET.SHEETS.EXPENSES);
  return !!expenses&&normalize_(expenses.getRange('A19').getDisplayValue())==='DESCRIZIONE'&&normalize_(expenses.getRange('C19').getDisplayValue())==='MOVIMENTO';
}
...
function refreshTasksPreserveTemplateV6_(eventId,event,child) {
  ...
  const backend=backendSheet.getDataRange().getValues().slice(1).filter(r=>String(r[1])===String(eventId));
  ...
  backend.forEach((r,index)=>{const depId=String(r[14]||'').trim(),status=normalize_(r[6]);if(!depId||status==='FATTO'||status==='COMPLETATA')return;sheet.getRange(index+2,5).setFormula(fastTaskStatusFormulaV7_(index+2));});
  // Riallinea le voci della colonna nativa DESCRIZIONE TASK con il catalogo centrale.
  ensureNativeTaskTableV4_(child,sheet);
  return out.length;
}

function ensureFastEventTaskTriggerV6_(child) {
  const sourceId=child.getId(),triggers=ScriptApp.getProjectTriggers(); let fastExists=false;
  triggers.forEach(t=>{let sameSource=false;try{sameSource=t.getTriggerSourceId()===sourceId;}catch(e){}if(!sameSource)return;const handler=t.getHandlerFunction();if(handler==='handleEventTaskEditV4'){ScriptApp.deleteTrigger(t);return;}if(handler==='handleFastEventTaskEditV6')fastExists=true;});
  if(!fastExists)ScriptApp.newTrigger('handleFastEventTaskEditV6').forSpreadsheet(sourceId).onEdit().create();
}
function fastTaskStatusFormulaV7_(row){return '=IF(A'+row+'="";"";IF(D'+row+'="";"DA FARE";IFERROR(IF(INDEX($E$2:$E$500;MATCH(D'+row+';$C$2:$C$500;0))="FATTO";"DA FARE";"IN ATTESA");"IN ATTESA")))';}
function handleFastEventTaskEditV6(e) {
  ...
  if(firstCol<=1&&lastCol>=1){const allNumbers=sheet.getRange(2,3,Math.max(1,Math.min(TASKS_V4.ENTRY_END_ROW,sheet.getMaxRows())-1),1).getValues();let nextNumber=allNumbers.reduce((max,row)=>{const n=Number(row[0]);return Number.isFinite(n)&&n>max?n:max;},0)+1;for(let row=firstRow;row<=lastRow;row++){const description=String(sheet.getRange(row,1).getDisplayValue()||'').trim();if(!description)continue;if(!(Number(sheet.getRange(row,3).getValue())>0))sheet.getRange(row,3).setValue(nextNumber++);autoLinkConfirmationTaskFastV7_(sheet,row);applyFastTaskDependencyV7_(sheet,row);}}
  if(firstCol<=4&&lastCol>=4)for(let row=firstRow;row<=lastRow;row++)applyFastTaskDependencyV7_(sheet,row);
...
function syncSelectedEventSheetToCalendarV2() {
  const ui=SpreadsheetApp.getUi(),event=selectedEvent_(),eventId=ensureEventId_(event),child=getLinkedEventSheetForSaveV2_(event);validateEventSheetIdentity_(child,eventId);
  if(isEventSheetV7_(child)){ui.alert('Salvataggio dalla Scheda evento','Questa è una Scheda evento di nuova generazione. Per salvare usa il menu “Scheda evento” → “💾 Salva dati nel Calendario” direttamente nel file evento.\n\nIn questo modo vengono gestiti correttamente preventivi, pagamenti multipli, residui e task automatiche.',ui.ButtonSet.OK);return null;}
  ensureChecklistBackendHeadersV3_();ensureParticipantsBackendHeadersV2_();
  const participantCount=syncParticipantsV3ToBackend_(eventId,event,child),taskCount=syncTasksV4ToBackend_(eventId,event,child),expenseCount=syncExpensesV4ToBackend_(eventId,event,child);ensureFastEventTaskTriggerV6_(child);SpreadsheetApp.flush();
  const counts={tasks:taskCount,participants:participantCount,expenses:expenseCount};ui.alert('Calendario aggiornato','I dati presenti nella Scheda evento sono stati salvati nel Calendario.\n\nAttività: '+counts.tasks+'\nSpese: '+counts.expenses+'\nPartecipanti: '+counts.participants,ui.ButtonSet.OK);return counts;
}
...
function initializeEventSheetV7FromCalendar_(child,eventId,event,folderId) {
  ...
  if(imp){const cal=sh_(APP.SHEETS.CALENDAR),map=headerMap_(cal),col=map[APP.CALENDAR_HEADERS.COMMITMENT];if(col&&event._row)cal.getRange(event._row,col).setValue(imp).setNumberFormat('@');const expenses=child.getSheetByName(EVENT_SHEET.SHEETS.EXPENSES);if(expenses){if(isEventSheetV8_(child))expenses.getRange('E9').setValue(imp).setNumberFormat('@');else expenses.getRange('N9').setValue(imp).setNumberFormat('@');}}

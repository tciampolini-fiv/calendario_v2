...
function eventSaveActivitiesV13_(master,child,eventId){
  const local=child.getSheetByName(EVENT_APP.SHEETS.TASKS);if(!local)throw new Error('Foglio Attività non trovato nella Scheda evento.');eventRefreshActivitiesV14_();
  const objectiveBackend=eventEnsureMasterObjectivesV13_(master),taskBackend=eventMasterSheet_(master,'_CHECKLIST');
  if(taskBackend.getMaxColumns()<23)taskBackend.insertColumnsAfter(taskBackend.getMaxColumns(),23-taskBackend.getMaxColumns());
  taskBackend.getRange(1,17,1,7).setValues([['ID OBIETTIVO','ORDINE STEP','SCADENZA AUTOMATICA','ID TASK PRECEDENTE','PERCORSO','MODALITA SCADENZA','OFFSET GIORNI']]);
...
function eventRefreshMasterActivitySummaryV13_(master,eventId,child){
  ...
  vals.forEach(r=>{if(eventNormalize_(r[7])==='OBIETTIVO'&&r[8])objectives.push({id:String(r[8]),name:String(r[0]||''),status:eventNormalize_(r[5]),order:Number(r[10]||0),tasks:[]});});vals.forEach(r=>{if(eventNormalize_(r[7])==='TASK'&&r[8]){const o=objectives.find(x=>x.id===String(r[8]));if(o)o.tasks.push({checked:Boolean(r[4]),status:eventNormalize_(r[5]),name:String(r[2]||''),step:Number(r[11]||0)});}});objectives.sort((a,b)=>a.order-b.order);objectives.forEach(o=>o.tasks.sort((a,b)=>a.step-b.step));const progress=objectives.map(o=>(o.status==='COMPLETATO'?'✓':o.status==='IN RITARDO'?'⚠':'•')+' '+o.name+'  '+o.tasks.map(t=>t.checked?'☑':'☐').join('')).join('\n'),actions=[];objectives.forEach(o=>o.tasks.filter(t=>t.status==='DA FARE').forEach(t=>actions.push(o.name+' — '+t.name)));if(advCol>0)cal.getRange(row,advCol).setValue(progress).setWrap(true).setVerticalAlignment('top');if(nowCol>0)cal.getRange(row,nowCol).setValue(actions.join('\n')).setWrap(true).setVerticalAlignment('top');
}

function eventTaskAutoKey_(description) {
  const d=eventNormalizeTextV7_(description);
  const direct={'RICHIESTA GOMMONE':'GOMMONE_RICHIESTA','CONFERMA GOMMONE':'GOMMONE_CONFERMA','RICHIESTA OSPITALITA':'CIRCOLO_RICHIESTA','CONFERMA OSPITALITA':'CIRCOLO_CONFERMA','RINGRAZIAMENTO':'CIRCOLO_RINGRAZIAMENTO','RICHIESTA SOGGIORNO':'ALLOGGIO_RICHIESTA','CONFERMA SOGGIORNO':'ALLOGGIO_CONFERMA','PAGAMENTO SOGGIORNO':'ALLOGGIO_PAGAMENTO','RICEZIONE FATTURA SOGGIORNO':'ALLOGGIO_FATTURA','INDIVIDUAZIONE SOLUZIONE PASTI':'PASTI_RICHIESTA','CONFERMA SOLUZIONE PASTI':'PASTI_CONFERMA','PAGAMENTO PASTI':'PASTI_PAGAMENTO','CONVOCAZIONE ATLETI':'CONV_ATLETI_INVIO','CONFERMA ATLETI':'CONV_ATLETI_CONFERMA','CONVOCAZIONE TECNICI':'CONV_TECNICI_INVIO','CONVOCAZIONE TECNICO':'CONV_TECNICI_INVIO','CONFERMA TECNICI':'CONV_TECNICI_CONFERMA','PRENOTAZIONE VIAGGIO':'VIAGGIO_PRENOTAZIONE','PAGAMENTO VIAGGIO':'VIAGGIO_PAGAMENTO','CONFERMA COMITATO DI ZONA':'ZONA_CONFERMA'};return direct[d]||'';
}
function eventTaskCategory_(description){const d=eventNormalize_(description);if(/CONVOC|ATLETI|TECNICI/.test(d))return'CONVOCAZIONI';if(/PAGAMENTO|FATTURA/.test(d))return'AMMINISTRAZIONE';if(/CIRCOLO|OSPITALITA|RINGRAZIAMENTO|COMITATO DI ZONA/.test(d))return'ORGANIZZAZIONE';if(/SOGGIORNO|HOTEL|ALLOGG|PAST|GOMMONE|TRASPORT|VIAGG|PRENOT/.test(d))return'LOGISTICA';return'ALTRO';}

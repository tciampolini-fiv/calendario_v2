...
function eventSaveTasks_(master,child,eventId) {
  const local=child.getSheetByName(EVENT_APP.SHEETS.TASKS);if(!local)throw new Error('Foglio Attività non trovato nella Scheda evento.');
  eventAutoLinkConfirmationTasksV9_();eventRefreshTaskDependencyStatesV9_();
  const backend=eventMasterSheet_(master,'_CHECKLIST'),oldRows=backend.getDataRange().getValues(),oldByNo={};let maxNo=0;
  for(let i=1;i<oldRows.length;i++){if(String(oldRows[i][1]||'')!==String(eventId))continue;const n=Number(oldRows[i][13]||0);if(n>0)oldByNo[n]=oldRows[i];if(n>maxNo)maxNo=n;}
  ...
  const rows=draft.map((x,index)=>{
    const r=x.values,old=x.old,depNo=Number(r[3]||0);if(depNo&&!idByNo[depNo])throw new Error('La task n. '+x.no+' dipende dalla task n. '+depNo+', che non esiste.');if(depNo===x.no)throw new Error('La task n. '+x.no+' non può dipendere da se stessa.');
    let status=eventNormalize_(r[4])||'DA FARE';if(!['DA FARE','IN ATTESA','FATTO'].includes(status))status='DA FARE';
    const completed=status==='FATTO'?(old[11] instanceof Date?old[11]:now):'',description=String(r[0]||'').trim(),autoKey=old[9]||eventTaskAutoKey_(description);
    const source=old[8]||(autoKey?'AUTO':'MANUALE');
    return[x.id,eventId,(index+1)*10,description,old[4]||eventTaskCategory_(description),r[5] instanceof Date?r[5]:'',status,old[7]||'',source,autoKey,String(r[1]||'').trim(),completed,now,x.no,depNo?idByNo[depNo]:'',status!=='FATTO'&&depNo?'DIPENDENZA':''];
  });
  eventReplaceRowsForEvent_(backend,eventId,2,rows,16);return rows.length;
}

function eventTaskAutoKey_(description) {
  const d=eventNormalizeTextV7_(description);
  const direct={
    'RICHIESTA GOMMONE':'GOMMONE','CONFERMA GOMMONE':'CONF_GOMMONE',
    'RICHIESTA SOGGIORNO':'SOGGIORNO','CONFERMA SOGGIORNO':'CONF_SOGGIORNO',
    'SOLUZIONE PASTI':'PASTI','CONFERMA PASTI':'CONF_PASTI','VIAGGIO TECNICO':'VIAGGIO_TECNICO',
    'CONVOCAZIONE ATLETI':'CONV_ATLETI','CONVOCAZIONE TECNICO':'CONV_TECNICO','CONFERMA PRESENZE TUTTI ATLETI':'CHECK_CONFERME',
    'OSPITALITA CIRCOLO':'OSPITALITA_CIRCOLO','CONFERMA OSPITALITA CIRCOLO':'CONF_OSPITALITA','RINGRAZIAMENTO CIRCOLO':'RINGRAZIAMENTO_CIRCOLO',
    'INVIARE CONTABILE AFOR':'CONTABILE_AFOR','PAGARE AFOR':'PAGARE_AFOR','RICHIEDERE FATTURA':'RICHIEDERE_FATTURA',
    'GOMMONE':'GOMMONE','CONTATTARE HOTEL':'SOGGIORNO','CONFERMA HOTEL':'CONF_SOGGIORNO','PASTI':'PASTI','CHECK CONFERMA PRESENZE':'CHECK_CONFERME'
  };
  return direct[d]||'';
}
function eventTaskCategory_(description){const d=eventNormalize_(description);if(/CONVOC|PRESENZ/.test(d))return'CONVOCAZIONI';if(/SOGGIORNO|HOTEL|ALLOGG|PAST|OSPITAL|GOMMONE|TRASPORT/.test(d))return'LOGISTICA';if(/PAG|FATTUR|RIF|AFOR|RIMBORS|CONTABILE|SALDO/.test(d))return'AMMINISTRAZIONE';if(/CIRCOLO|ZONA|COMUNIC/.test(d))return'ORGANIZZAZIONE';return'ALTRO';}
